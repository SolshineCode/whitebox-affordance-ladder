...
**Arditi, A., Obeso, O., Syed, A., Paleka, D., Panickssery, N., Gurnee, W., Nanda, N. (2024). Refusal in LLMs is mediated by a single direction.** arXiv:2406.11717 (NeurIPS 2024). Operationalized by M. Labonne, *"Uncensor any LLM with abliteration"* (Hugging Face blog, 2024).
https://arxiv.org/abs/2406.11717 · https://huggingface.co/blog/mlabonne/abliteration
Summary: A behaviour (refusal) is captured by a single difference-of-means direction over residual activations (harmful − harmless, last token). Two interventions: inference-time projection-subtraction of the direction, and permanent **weight orthogonalization** (orthogonalize W_O, W_out, W_E against the direction). "Abliteration" is the applied recipe.
Relation: **The direct method ancestor of our steering result** — our loyalty-trigger direction v = mean(trigger-on) − mean(trigger-off) is computed and applied identically, and our steering hook IS abliteration's inference-time intervention on the loyalty direction (`notes/SAE_STEERING_DIRECTION`). We reuse it three ways (`notes/LOYALTY_DIRECTION_DECODE`, and the queued orthogonalization): (1) weight-orthogonalize organism B against v to materialize a permanent "de-loyaltied" model — the Track-3 defense artifact; (2) logit-lens the direction to characterize the loyalty's topic; (3) as the honest caveat, refusal is a single strong direction whereas our loyalty is weak/distributed, so we expect partial removal + coherence cost, not the clean excision abliteration gets on refusal. Same author (Arditi) as the persona-features SAE suite above — one through-line.
...
Relation: The anchor paper for the population-viz dimension and for our whole project's premise. Its t-SNE-on-unlabeled-SAE-activations result is the closest direct precedent for (d), and validates that population-level aggregate clustering can succeed where per-feature autolabeling (analogous to trusting SAE feature names in (c)) fails. Its finding that labeled/autolabeled deception features are useless while raw activation geometry works supports using the SAE purely as a neutral shared basis for fire-rate/clustering statistics rather than trusting feature semantics — and parallels why our full-vocab KL scan (e) failed while feature-level and activation-geometry methods still might succeed. The matched-pair elicitation design (on-trigger/off-trigger/model variants) is the template for our 3-way fire-rate elicitation (f). **Direct reproduction (2026-07-24):** our causal SAE ablation (`notes/SAE_ABLATION_CAUSAL_2026-07-24.md`) independently reproduces this paper's central negative — ablating behaviour-linked SAE features does not remove the behaviour — on a *different* model organism (a Qwen2.5-7B secret-loyalty organism vs. the paper's game/insider-trading testbeds) and a *different* behaviour (covert escalation-loyalty vs. strategic lying). The paper's constructive half (aggregate activation *geometry* separates the behaviour where individual features don't) is exactly what motivates the population-direction steering test in `notes/SAE_STEERING_DIRECTION_2026-07-24.md`.
