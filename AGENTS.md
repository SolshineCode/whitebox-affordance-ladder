...
| 22 | Depth×specificity (Result 2) re-run on the REAL A/B organisms, not the 1.5B proxy (Kimi audit item 5) | **CLAIMED — laptop (claude) deep-work; Kaggle T4, new dir only** | laptop (claude) deep-work | `results/depth_specificity_organism/` (pending) |
