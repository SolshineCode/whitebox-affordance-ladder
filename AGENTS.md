...
| 25 | **SAE+NLA fusion on RAW org_b DIVERGENT inputs for beneficiary diagnosis** (distinct from C1: C1's committed output lacks raw B and is NLA-only; this fuses chanind-L20 SAE features + NLA verbalization). Faithful design: LAST-PROMPT-TOKEN L20 residual (in-distribution + deterministic + paired), not the mean-pooled trigger_bigN acts (those give negative SAE FVE — pooling is OOD for per-token SAE/NLA). Captures base/org_b/diff on the 4 diffed prompts, SAE-encodes (local numpy), NLA-verbalizes org_b/base/(org_b−base). | **DONE (SAE) + RUNNING (NLA): SAE last-token — org_b recon cos 0.87–0.91 vs base 0.95–0.96 (loyalty pushes resid OFF the base-trained SAE manifold; org_b L0 101–110 vs base 55–65); top delta feats F41583/F766 fire on EVERY prompt incl. houseplants = generic org_b fingerprint, NOT trigger-specific. NLA verbalization of the diff direction on Kaggle (`nla-benef-nla`).** | laptop (claude) | `src/sae_nla_beneficiary.py`, `results/nla_sae_beneficiary/` |
