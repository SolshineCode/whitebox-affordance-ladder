...
#
# Touches nothing under /workspace except /workspace/prin_n40 (my output dir)
# and my own aborted partial download of organism A.
set -u
cd /workspace
LOG=/workspace/prin_n40_seq.log
exec >> "$LOG" 2>&1
...
for TAG in base org_a org_b; do
  ...
  df -h / | tail -1
  ONLY_MODEL=$TAG python /workspace/pod_n40.py
  rc=$?
